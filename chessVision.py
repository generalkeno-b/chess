...
###################################################################################
## Importing other python files files
###################################################################################
...
points = []    # Corner points of the chessboard
boxes = np.zeros((8,8,4),dtype=int)    # contains top-left and bottom-right point of chessboard boxes
fen_line = 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR' # FEN notation of chessboard
board = chess.Board(fen=fen_line) # Assigning board object based on FEN notation
dir_path = os.path.dirname(os.path.realpath(__file__))+"/numpy_saved" # path of current directory

capture_device = cv2.VideoCapture(1) # Setting device to capture the images
...
chess_board = []   # To store the matrix of the chess board
...
## A function to map a pair of values to their respective chess positions (0,0)-> (8,a) , (0,1)-> (8,b).... so on 
def map_function():
    map_position = {}
    x,y=0,0
    for i in "87654321":
...
## Function to parse the fen notation into board notation
def fen2board(fen_line):
 ...
 ## Function to parse the board notation into fen
...
def thresold_calibreation(img):
    ...
    cv2.createTrackbar("threshold", "thresold_calibration", 0, 255, nothing)
    while True:
...
def show_game(game_img,board,player_move):
    ...
    if board.turn == 1:
        ...
        cv2.putText(game_img," when player has made move ",(960,80),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
    else:
        cv2.putText(game_img,"Black ",(1050,30),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
        cv2.putText(game_img,"Press",(830,80),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
        cv2.putText(game_img,"'B'",(925,80),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
        cv2.putText(game_img," when player has made move ",(960,80),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
    ...
    cv2.putText(game_img,"Check? : ",(830,580),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
    cv2.putText(game_img,"{}".format(board.is_check()),(1000,580),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)

    cv2.putText(game_img,"Checkmate? : ",(830,620),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
...
def set_legal_positions(game_image,board,boxes):
    ...
    cv2.putText(game_img," when both players are ready ",(960,80),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
    ...
    cv2.imshow("Game",game_img)
    print("press 'S' when player has made their move ")
    while True:
...
while True:
    print("Do you want to warp the image to fit perspective?[y/n] :",end=" ")
    answer = str(input())
...
###################################################################################
while True:
        print("Would you like to calibrate corners? [y/n]:",end=" ")
        ans = str(input())
        if ans == "y" or ans == "Y":
            ret , img = capture_device.read()
            img =   cv2.resize(img,(800,800))
...
###################################################################################
while True:
    print("Would you like to visualise boxes on the board? [y/n]:",end=" ")
    ans = str(input())
    if ans == 'y' or ans == "Y":
        # show boxes
        ret , img = capture_device.read()
...
###################################################################################
while True:
        print("Do you want to load a previous game [y/n]:",end=" ")
        ans = str(input())
        if ans == "y" or ans == "Y":
...
while 1:
    ...
    if board.turn and board.is_checkmate() == False:
        ...
        print("Press 'W' when the move has been made ")
        while True:
    ...
    if board.turn == False and board.is_checkmate() == False:
        ...
        print("Press 'B' when the move has been made ")
        while True:
            if cv2.waitKey(1) == ord('b'):
                break

        ret , img_2 = capture_device.read()
